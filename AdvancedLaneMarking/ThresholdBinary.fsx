...
let experimentalLines() = 
    ...
    let m = getTransform()
    let minv = m.Inv()
    let i = Cv2.ImRead( @"D:\repodata\adv_lane_find\imgs\img576.jpg")
    let warped = new Mat()
    ...
    i,warped,minv,leftPoints,rightPoints,leftMids,rightMids,lpx,rpx,lpxT,rpxT,lineLeft,lineRight

//experiment with image processing (not used)
let experimental() =
    let i,warped,minv,leftPoints,rightPoints,leftMids
        ,rightMids,lpx,rpx,lpxT,rpxT,
